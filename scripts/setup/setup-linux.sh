...
# ============================================
# PhoneticHybrid Setup Script - Linux
# ============================================
...
echo ""
echo "╔════════════════════════════════════════╗"
echo "║  PhoneticHybrid Setup - Linux          ║"
echo "╔════════════════════════════════════════╗"
echo ""
...
if ! command -v espeak-ng &> /dev/null; then
    echo -e "${YELLOW}eSpeak-NG not found. Installing...${NC}"
    ...
    if command -v apt-get &> /dev/null; then
        ...
        echo -e "${RED}Error: Could not detect package manager. Please install espeak-ng manually.${NC}"
        exit 1
    fi
...
echo ""
echo -e "${YELLOW}Installing system dependencies for audio processing...${NC}"
...
if command -v apt-get &> /dev/null; then
    ...
    sudo pacman -S --noconfirm libsndfile ffmpeg
fi
...
echo "     ./scripts/start/start-linux.sh"
echo ""
